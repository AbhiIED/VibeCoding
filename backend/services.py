...
async def fetch_30d_history(base: str, target: str, db=None) -> list:
    base, target = base.upper().strip(), target.upper().strip()
    end_date = datetime.now().date()
    start_date = end_date - timedelta(days=30)

    if base == target:
        # Generate 30 days of 1.0 rate
        result = []
        for i in range(31):
            d = (start_date + timedelta(days=i)).isoformat()
            result.append({"date": d, "rate": 1.0})
        return result
    ...
    # fetch current live rate and construct realistic trend points
    current_rate = 1.0
    if db:
        try:
    ...
    for i in range(30):
        ...
        history.append({"date": d, "rate": round(current_rate * factor, 4)})
    history.append({"date": end_date.isoformat(), "rate": round(current_rate, 4)})
    return history
